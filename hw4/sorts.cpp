...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
   ...
   // if (s.size() == 2  &&  comp(s[1], s[0]))
    ...
    {
        int loc = unsorted;
        while ((loc >= 0) && comp(s[unsorted], s[unsorted-1]))//(s[unsorted-1].avg > key))
        {
            swap(s[unsorted-1], s[unsorted]);//s[loc].avg = s[loc-1].avg;
            loc--;
